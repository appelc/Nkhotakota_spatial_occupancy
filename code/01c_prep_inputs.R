...
occ_covar <- readRDS('data/cleaned/occ_covar.RDS')
  str(occ_covar)

coords <- readRDS('data/cleaned/coords.RDS')
  head(coords)
...
(nseasons <- 3) #this is wet/cool/dry
...
#now that we've removed a couple seasons, make sure all sites still have data
...
## THERE IS ONE SITE WITH NO DATA NOW THAT WE REMOVED TWO SEASONS. WE'LL REMOVE IT BELOW.


## Format OCC covariates for input ---------------------------------------------
...
str(occ_covar_clean) #should be length n_sites-1 now

#create numeric version of SITE (0-indexed
...
## Format DET covariates for input ---------------------------------------------
...
det_covar_clean = list('season' = occ_covar_clean$season,
                       ...
                       'year' = occ_covar_clean$year)

str(det_covar_clean) #should be correct length already
...
head(det_covar_clean$yearsn_2024cool)

str(det_covar_clean)


## Format coordinates for input ------------------------------------------------
...
str(coords_clean) #length n_sites-1
...
#save modified
saveRDS(coords_clean, 'data/cleaned/coords_cleaned.RDS')
...
#detections
head(inputs[[1]]$y,10)
dim(inputs[[1]]$y) #200 sites, 17 year-seasons, 22 weeks
...
#ensure no NAs in occurrence covariates
...
#save
saveRDS(inputs, 'data/cleaned/sp_inputs.RDS') 


## Now proceed to 02a_run_model.R
